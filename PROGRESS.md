...
- PostgreSQL 遷移 [進行中]
  ...
  - 收尾與部署 [進行中]
    ...
    - 本機 SQLite 測試檔案資料過期 [待處理] (8/20) — `database/drink-group-buy-dev.sqlite` 目前資料跟現在的 `seed-dev.sql` 對不上（`store-001` 查不到），2026-08-20 用 `scripts/order-api-smoke.js` 測試時發現；修好需要用 `db:init` + `db:seed` 整個重建這個檔案，屬於會改變本機檔案狀態的操作，先記錄、待確認後再處理；詳見 `database/README.md`
