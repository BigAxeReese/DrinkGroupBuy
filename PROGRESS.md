...
- 視覺風格：奶茶上色，不使用藍色 [進行中]
  ...
  - 風格方向與統一設定 [完成] (9/20) — 使用者比較深鈷藍、淺藍、霧藍、黑糖褐四組後選定「不用藍、黑糖褐」；數值放在 [tokens.js](mobile/src/theme/tokens.js) 與 [statusTones.js](mobile/src/theme/statusTones.js)，新增 10 項自動檢查（文字看不看得清楚、狀態顏色分不分得開、每個狀態都有對應顏色），mobile 測試 53/53 全過。尚未套用到任何畫面
    > 把選定的顏色、字級、圓角，以及 33 種狀態標籤各自用哪種顏色，寫成畫面可以直接引用的統一設定，並且用自動檢查確保文字看得清楚。
  - 把畫面裡寫死的顏色換成統一設定 [待處理]
    > 目前有 32 個檔案、共 565 處各自寫死的顏色，逐檔改成引用統一設定；這一步做完，畫面外觀應該完全不變。
  - 共用元件換上新配色 [待處理]
    > 按鈕、頁面標題、狀態標籤、底部導覽這幾個到處共用的元件換上新配色，其他畫面會一起跟著變。
  - 珍珠進度盤 [待處理]
