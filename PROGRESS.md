...
- 即時地圖店家標記客製化外觀（含店名） [進行中] (9/14)
  - 第一版：文字標籤疊層（不動 Marker 本身） [完成] (9/14) — `LiveMapScreen.native.jsx` 新增店名文字標籤，疊在圖釘下方常駐顯示（不用點下去才看到）。刻意不用 `react-native-maps` 的自訂 Marker 內容（把 View 塞進 `<Marker>`）：這個專案先前在 8/19 升級 Expo SDK 57 時，已經在真機上實際踩過「`react-native-maps` 在新版渲染架構（New Architecture）下顯示異常」的問題（地圖整個空白，詳見上方「Expo SDK 57 升級」），而自訂 Marker 內容正是已知在 New Architecture 下容易出問題的用法；這一版改用另一條路：圖釘維持原生內建的 `pinColor`（已驗證穩定），店名文字另外用普通的 `<View>`／`<Text>` 疊在地圖最上層，透過 `mapRef.pointForCoordinate()` 換算螢幕座標定位。`npm test` 138/138 全過。**9/14 真機驗證（Android，發完 EAS Update 後）**：地圖正常顯示，未出現先前 SDK 57 那種空白/異常，店名標籤正確顯示在對應圖釘下方；已知限制是拖曳過程中標籤跟不上圖釘、放開手勢才追上，使用者實測確認可接受。
  - 第二版：改用真正的自訂 Marker（圖釘外觀本身客製化，不只是外掛文字） [進行中] (9/14) — 使用者確認想要的是「跟 Web 版一樣客製化圖釘外觀」（圓形色塊＋文字，不只是原生水滴圖釘外掛一行字），這次改回第一版刻意避開的那條路：`<Marker>` 直接塞自訂 `<View>`（圓形徽章＋店名標籤，樣式比照 `LiveMapScreen.web.jsx` 的 `StoreOverlayMarker`），移除第一版的 `pointForCoordinate` 疊層機制（不再需要，標籤現在是 Marker 內容的一部分）。**使用者已明確知悉且承擔風險**：這是這個專案先前在 SDK 57 升級時真的踩過的那個問題類型（`react-native-maps` 自訂內容在 New Architecture 下的已知不穩定用法），第一版才刻意繞開；這次是使用者在了解這個風險、也看過第一版真機驗證成功之後，明確要求改用這個做法。
    - 首次真機測試（9/14）：地圖本身正常（沒有變空白），**但所有圖釘完全不顯示**——找到可能原因：初版加了 `tracksViewChanges={false}`（社群常見的效能緩解手段，避免 Android 每一幀重繪自訂 Marker），但這個屬性如果從第一次渲染就是 `false`，Android 端會直接跳過「把 View 拍成圖片」這個步驟，圖釘等於從來沒有生成過圖片，整顆消失——這是 `react-native-maps` 另一個已知的坑，跟一開始擔心的「New Architecture 顯示異常」是不同的失敗模式，但一樣是這條路徑本身的脆弱之處。已移除 `tracksViewChanges={false}`（改回套件預設值 `true`，犧牲一點重繪效能換取圖釘至少會顯示），`node --check`／`npm test` 138/138 全過。**這個快修尚未經過真機驗證**，需要再發一次 EAS Update 請使用者重新測試。
  > 目前地圖上的店家標記只有顏色（有沒有招募中團購），沒有常駐顯示店名，要點下去才看得到；這次改成不用點也能直接看到店名。
