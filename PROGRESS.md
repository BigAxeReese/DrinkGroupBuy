...
- 取餐結束時間改為系統自動計算 [完成] (8/20)
  ...
  - 商家設定打烊時間的介面 [完成] (9/16) — 呼應上面「已知缺口」：新增 `PATCH /api/merchant/stores/:storeId/pickup-closing-time`（SQLite／PostgreSQL 兩條寫入路徑皆有接上，重用既有 `merchantMenuRepository` 的店家歸屬檢查與 audit log 慣例），格式驗證統一在後端做（24 小時制 `HH:MM`，00:00-23:59；SQLite 本身的 CHECK 只擋得住格式、擋不住超出範圍的小時數，所以應用層另外驗證，兩邊資料庫行為才會一致），留空即清除＝恢復 24 小時營業。商家菜單管理畫面（`MerchantMenuManagementScreen.jsx`）新增「打烊時間設定」區塊，會先帶出目前設定值。已用真實 PostgreSQL 16 走完整 HTTP 流程驗證：設定成功、格式錯誤拒絕（400）、跨店商家嘗試修改被拒絕且不影響原值（403）、清除成功、每次成功異動各留一筆 audit log；另外用假資料庫做的 repository 單元測試涵蓋 sqlite 委派、postgres 成功／越權／店家不存在四種情境。`npm test` 92 項全數重跑通過（另有 3 項與此次改動無關的既有失敗，經 `git stash` 對照確認在改動前就已存在）
    > 前面提過「沒有畫面可以設定店家打烊時間」這個缺口，現在補上：商家可以在菜單管理畫面直接輸入自己店的打烊時間（或留空代表 24 小時營業），系統會用這個時間去擋「取餐窗口不能超過打烊時間」那條規則，不用再直接改資料庫。
