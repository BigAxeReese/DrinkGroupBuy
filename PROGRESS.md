...
- 視覺風格：紫青配色 [進行中]
  ...
  - 換用紫青配色 [進行中] (9/24) — 使用者從配色網站挑了一組四色（深紫、灰藍、青、薄荷），比較過藍綠、暖粉彩、鼠尾草橙、紫青、藍共 9 組並排預覽後選定紫青，取代 9/20 的黑糖褐（「App 內不使用藍色」的規則一併作廢）；只改 [tokens.js](mobile/src/theme/tokens.js) 的設定值，所有已換新樣式的顧客畫面自動換色，六種狀態標籤重新配色，登入頁也改成引用同一套設定（只換顏色，按鈕形狀待重做）。自動檢查 10 項全過（狀態標籤底色最小色差 15，門檻 8），並用網頁預覽看過登入、首頁、個人中心。**尚未做的**：Android 實機確認、按鈕樣式重做（使用者決定另外處理）、登入插圖與桌面圖示重新上色；青色「進行中」標籤是所有標籤裡最亮的一顆，需要實機確認會不會太搶眼。
