...
- 依賴套件安全 [進行中]
  ...
  - 已知漏洞修復（非強制） [進行中] — Root 11 項（6 中 5 高）、Mobile 46 項（1 低 11 中 33 高 1 重大），剩餘需 Expo／React Native 主版本升級
    > 用工具掃出目前用到的套件裡有哪些已知漏洞，目前掃到不少項目，但修復通常需要把 Expo／React Native（開發這個手機 App 用的框架）升級到新的大版本，屬於影響範圍比較大的改動，不是馬上就要做。
