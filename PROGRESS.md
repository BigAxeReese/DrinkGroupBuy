...
- 商家自助取消團購 [進行中]
  ...
  - 管理員舊版取消團購工具資料不完整 [完成] (8/20) — 改為重用商家自助取消已驗證過的連動邏輯（含店家歸屬檢查改為一律放行、audit log 動作類型保留原本的管理員專屬標籤）；已對真實 PostgreSQL 16 用真實 HTTP 流程驗證訂單取消／付款作廢／audit log 三者皆正確；API 回應也一併修正為轉發完整結果（原本只回傳活動本身，看不出底下有沒有訂單處理失敗）。刻意取捨：管理員取消現在也會受「活動必須是招募中」「截止前 30 分鐘鎖定」這兩條原本只有商家自助取消才有的限制，換取正確的訂單/付款連動；如果之後需要管理員能略過這兩條限制，需另外設計專屬機制，記錄於 2026-08-20 `docs/AI-security-review-log.md`
    > 工程師內部用、非正式顧客／商家會用到的舊版取消工具，取消團購時只會改活動狀態，不會一併取消底下的訂單、也不會撤銷已經預先鎖定的顧客付款，會留下資料兜不起來的訂單。
- 附近公里數篩選 [完成]
...
- LINE Pay 付款 [進行中]
  ...
  - 核心請款流程 [進行中]
    ...
    - 付款前取餐／逾期未取規則同意 [進行中]
      ...
      - Android + LINE Pay sandbox 人工 E2E [進行中] (8/15) — 部分驗證：Android 長文排版、未勾選停用、勾選後送出，以及抵達 LINE Sandbox 登入頁已人工驗證；資料庫保存 1 筆 v1.0 規則全文快照與 1 筆 pending 預授權。尚未輸入 LINE 測試帳密或執行授權 confirm／App 回跳，因此不可視為完整付款 E2E
