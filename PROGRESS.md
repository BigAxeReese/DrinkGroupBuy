...
- 即時地圖店家標記下方顯示店名 [進行中] (9/14) — `LiveMapScreen.native.jsx` 新增店名文字標籤，疊在圖釘下方常駐顯示（不用點下去才看到）。**刻意不用 `react-native-maps` 的自訂 Marker 內容**（把 View 塞進 `<Marker>`）：這個專案先前在 8/19 升級 Expo SDK 57 時，已經在真機上實際踩過「`react-native-maps` 在新版渲染架構（New Architecture）下顯示異常」的問題（地圖整個空白，詳見上方「Expo SDK 57 升級」），而自訂 Marker 內容正是已知在 New Architecture 下容易出問題的用法；這次改用另一條路：圖釘維持原生內建的 `pinColor`（已驗證穩定），店名文字另外用普通的 `<View>`／`<Text>` 疊在地圖最上層，透過 `mapRef.pointForCoordinate()` 把店家經緯度換算成螢幕座標定位，地圖平移/縮放時（`onRegionChangeComplete`）重新計算一次位置——這是跟畫面上其他浮動按鈕（篩選、回到目前位置）相同的疊層技術，不會經過已知不穩定的那條自訂 Marker 路徑。`node --check`／babel 語法檢查通過，`npm test` 138/138 全過（不影響既有測試涵蓋的邏輯）。**尚未經過任何實機或模擬器驗證**：這個環境沒有 Android 裝置可以測，Web 預覽走的是完全不同的 `LiveMapScreen.web.jsx`（本來就沒有這個問題，不能拿來驗證這次改動），純粹只確認了程式碼語法正確、不會讓其他測試掛掉；地圖拖曳時標籤位置更新是非同步的，理論上會比圖釘本身的移動慢一點點，這點也還沒有人工看過順不順。使用者已知悉這次改動有導致地圖顯示異常的風險，要求開發前先記錄。
