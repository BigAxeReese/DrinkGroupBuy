...
- 即時地圖店家標記客製化外觀（含店名） [完成，維持文字疊層版] (9/14)
  ...
  > 目前地圖上的店家標記只有顏色（有沒有招募中團購）＋常駐店名文字，圖釘形狀本身是原生水滴圖釘，不是像 Web 版那樣完全自訂的圓形色塊——這是已知、刻意維持的限制，原因見上方第二版嘗試的記錄。
...
- 特定裝置閃退：三星 Galaxy S24+ [待查] (9/14) — 使用者的組員在 S24+ 上測試時，App 反覆閃退，Android 系統跳出「屢次停止運作」提示（有截圖佐證，發生在剛開啟 App、回到桌面前這段，不確定是啟動流程本身還是特定畫面觸發）。**同一批版本在其他三台裝置都正常**：使用者自己的 S25、測試用的 A54、Tab S9 FE 平板皆未重現，範圍已縮小到這支特定裝置（或這個特定安裝）。已請使用者的組員依序嘗試：(1) 強制停止＋重開確認抓到最新 EAS Update（截圖確認是在 9/14 最後一次「還原地圖圖釘」更新之後拍的，已排除是更新前的舊版本）、(2) 解除安裝＋重新安裝乾淨測試。**目前沒有 `adb logcat` 的實際錯誤堆疊，無法確認根因**——不排除是這支裝置的既有相容性問題，也不排除跟近期地圖圖釘改動（雖已還原）殘留的某種狀態有關；使用者已表示先擱置，之後有結果或方便取得 logcat 再繼續查。
