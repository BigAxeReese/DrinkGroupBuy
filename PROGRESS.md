...
- 即時地圖店家標記客製化外觀（含店名） [進行中] (9/14)
  ...
  - 第二版：改用真正的自訂 Marker（圖釘外觀本身客製化，不只是外掛文字） [進行中] (9/14) — 使用者確認想要的是「跟 Web 版一樣客製化圖釘外觀」（圓形色塊＋文字，不只是原生水滴圖釘外掛一行字），這次改回第一版刻意避開的那條路：`<Marker>` 直接塞自訂 `<View>`（圓形徽章＋店名標籤，樣式比照 `LiveMapScreen.web.jsx` 的 `StoreOverlayMarker`），移除第一版的 `pointForCoordinate` 疊層機制（不再需要，標籤現在是 Marker 內容的一部分）。有加上業界已知的緩解手段 `tracksViewChanges={false}`（避免 Android 每一幀重新繪製自訂 Marker 造成的效能/閃爍問題，是社群公認的標準做法，但不保證完全避開 New Architecture 下的顯示問題）；代價是店家招募狀態或店名如果在畫面停留期間變動，圖釘不會即時更新外觀，要離開再進入這個畫面才會重繪。`node --check`／babel 語法檢查通過，`npm test` 138/138 全過。**使用者已明確知悉且承擔風險**：這是這個專案先前在 SDK 57 升級時真的踩過的那個問題類型（`react-native-maps` 自訂內容在 New Architecture 下的已知不穩定用法），第一版才刻意繞開；這次是使用者在了解這個風險、也看過第一版真機驗證成功之後，明確要求改用這個做法。**尚未經過真機驗證**——需要重新發一次 EAS Update 並實機確認地圖沒有變空白/異常、圖釘外觀正確顯示。
