...
| Cart/order draft candidate | `editing`, `submitted`, `expired`, `cancelled` | Customized drinks remain editable until cart submission creates an order | DrinkSelectionScreen, CartScreen | `editing -> submitted/cancelled/expired` | `submitted -> editing` without a new draft | Maybe | Current prototype does not store a cart status; it only holds runtime items |
...
- 訂單金額變動時，prototype 使用 `authorized -> pending` 表示需要重新預授權；正式狀態可能需要獨立的 `reauthorization_required`，避免與首次預授權混淆。
- 活動截止前 30 分鐘進入 order lock：活動仍為 `recruiting` 且可加入，但既有訂單不可修改或退出；正式設計可能需要獨立的 `order_locked` capability，而不是新增活動 status。
