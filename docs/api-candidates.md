...
| MerchantDealCreateScreen | Create merchant deal and return dashboard summary | POST | `/mobile/merchant/deals` | `storeId`, `title`, `targetCups`, `discountAmount`, `maximumCups`, `endTime`, `pickupTime`, `notices` | Created deal and merchant dashboard summary | Merchant store ownership, tier validation, time validation | Multi-tier create form still unresolved |
