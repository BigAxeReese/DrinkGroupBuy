...
3. `capture` 失敗時，顧客端顯示「扣款失敗」並可重新付款。
4. `capture` 失敗時，店家端仍可看到該筆訂單，但顯示灰色「待付款」，且不列入製作清單。
5. 顧客重新付款成功後，顧客訂單、店家訂單與團購整體改為「製作中」，該筆訂單加入製作清單。
6. 顧客若一直沒有重新付款，顧客端維持「扣款失敗」，店家端維持灰色「待付款」，到取餐時間結束後移到歷史訂單。
7. 團購截止後正式扣款期間，顧客端不顯示「付款處理中」或「結算中」，仍顯示「訂單已鎖定」。
8. 每次重試前，系統必須先查詢或確認目前 provider 狀態，避免重複請款或重複取消授權。
9. `void` 失敗時，系統持續重試；若授權自然過期，可視為釋放完成，但必須保留 provider event 與 status history。
10. 若系統仍無法判斷最終狀態，保留失敗狀態與稽核紀錄；這不是一般使用者操作流程，也不先做管理員介面。
