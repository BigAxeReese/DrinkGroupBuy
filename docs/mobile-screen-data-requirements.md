...
| `PaymentReportScreen`       | 模擬 LINE Pay 授權 / 請款，並開啟 sandbox 授權頁 | 原價金額、授權金額、最終金額、請款金額、釋放金額、付款狀態、provider `transactionId`、`paymentUrl`、backend 結果 | 開啟 LINE Pay sandbox 授權 URL、自動刷新 backend 訂單狀態、手動刷新付款狀態、使用 mock fallback、revision 重新預授權 | Mobile payment state；`POST /api/payments/line-pay/request`；`GET /api/orders/:orderId` | 畫面命名調整、正式 app deep link、webhook / 錯誤恢復、refund；void/capture 已先在 backend payment module 處理 |
| `GroupProgressScreen`       | 顯示團購與顧客訂單進度                           | 團購徽章、目前 / 下一門檻杯數、參與者、剩餘時間、訂單摘要                                                        | 前往付款或取餐資訊                                                                                          | Mobile app state                                                                        | 進度 API、權威結算結果                                                                                 |
| `CustomerOrdersScreen`      | 顧客查看進行中 / 歷史訂單與編輯訂單              | 店家、品項、客製化、品項金額、訂單總額、訂單狀態、取餐碼                                                         | 開啟團購、鎖單前編輯 / 刪除、建立 revision、前往重新授權                                                   | Mobile orders/payments/localStorage；`POST /api/orders/:orderId/revisions`              | 訂單列表改接後端、revision 錯誤提示與退出團購 API                                                       |
...
| `MerchantDealCreateScreen`  | 商家建立團購與優惠門檻                           | 固定店家、標題、24 小時內截止時間、取餐時間、公告、優惠門檻                                                      | 新增 / 刪除門檻、建立團購；截止時間超過 24 小時時阻擋送出                                                    | POST API + local fallback                                                               | 更完整的取貨時間驗證、重試 UX                                                                          |
