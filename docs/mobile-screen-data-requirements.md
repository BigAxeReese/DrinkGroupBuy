...
| `LiveMapScreen`             | 用地圖瀏覽活動店家                               | Google Map、由 Backend 活動店家座標建立的 marker、店名、地址、進行中的團購杯數進度；dev auth mode 依登入 Backend `userId` 套用控制台固定位置或 GPS | 地圖拖曳／縮放、查看 marker、進入活動詳情或店家菜單 | Google Maps SDK + `GET /api/group-buy-activities` 的 `store`；dev-only `GET /api/app/config?userId=...` + `expo-location`；不再讀取匯出／mock 店家 runtime | 正式版定位／隱私流程、`GET /api/stores/nearby`、距離、無活動店家、活動同步失敗提示與 Android E2E |
